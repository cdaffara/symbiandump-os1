...
_LIT(KServerName,"PARAM_SVR_SUITENAME_sc");
...
// __EDIT_ME__ - Use your own server class name
	...
	server->RetrieveUpsAuthorisationGrantedL();
	...
	if (err == KErrNone)
		...
		if (size < 1024)
			...
			if (buf.FindF(_L("SessionYes")) != KErrNotFound ||
				buf.FindF(_L("Yes")) != KErrNotFound ||
				buf.FindF(_L("Session")) != KErrNotFound ||
				buf.FindF(_L("Always")) != KErrNotFound)
				{
				iUpsAuthorisationGranted = ETrue;
				}
...
PARAM_FOREACH_MESS_BEGIN
		if (aStepName == _L("CPARAM_MESS_NAMEStep"))
			...
			testStep = new CPARAM_MESS_NAMEStep(iUpsAuthorisationGranted);
...
enum TVerdict CCapabilityTestStep::doTestStepL()
	...
	if( SR_MESSAGE_ID == ECNControl )
		{
		INFO_PRINTF3(_L("Test for IPC message ECNControl, Level %d, Option %d - Starting ..."), iControlLevel, iControlOption & ~(KConnInternalOptionBit | KConnWriteUserDataBit | KConnReadUserDataBit));
		}
	else
		{
		INFO_PRINTF2(_L("Test for IPC message %d - Starting ..."), SR_MESSAGE_ID);
		}
