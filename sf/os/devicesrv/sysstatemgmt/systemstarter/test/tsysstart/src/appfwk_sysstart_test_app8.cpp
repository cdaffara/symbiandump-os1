...
_LIT(KSysStartTestApp8Panic, "SysStartTestApp8");
...
LOCAL_C void StartTestL(void)
	{
	RDebug::Print(_L("appfwk_sysstart_test_app8::StartTestL - entry"));
	
	TAppfwkSysstartTestAppResults myResults;
	CAppfwkSysstartTestAppUtils::ReadTestAppResultsFileL(KTApp8ResultFileName, myResults);
	...
	if ((app7Results.iRendCount==0) && (app7Results.iStartCount==1))
		{
		myResults.iStartCount++;
		}
		
	myResults.iRendCount++;
	CAppfwkSysstartTestAppUtils::ReplaceTestAppResultsFileL(KTApp8ResultFileName, myResults);
	RProcess::Rendezvous(KErrNone);
	
	RDebug::Print(_L("appfwk_sysstart_test_app8::StartTestL - exit"));
	}
