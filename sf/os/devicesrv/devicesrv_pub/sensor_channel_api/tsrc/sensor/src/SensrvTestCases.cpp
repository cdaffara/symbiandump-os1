    ...
    switch( aTestCase )
        ...
        case EAddConditionSet:
            ...
            if( !err )
                ...
                optErr = aItem.GetNextInt(iConditionSetRead);
...
TInt CSensrvTestCases::FindChannelsL()
    ...
    if( err )
        {
        RDebug::Print( _L("CSensrvTestCases::FindChannelsL: FindChannels error: %d"), err );
        return err;
        }
...
TInt CSensrvTestCases::FindChannels( RSensrvChannelInfoList& aChannelInfoList, TSensrvChannelInfo& aSearchConditions )
    {
    RDebug::Print( _L("CSensrvTestCases::FindChannels: Find channels...") );
    if( !iChannelFinder )
        {
        // Create channel finder
        RDebug::Print( _L("CSensrvTestCases::FindChannels: Create iChannelFinder") );
        iChannelFinder = CSensrvChannelFinder::NewL();
        }
    ...
    if( err )
        {
        RDebug::Print( _L("CSensrvTestCases::FindChannels: Leave: %d"), err );
        return err;
        }
    RDebug::Print( _L("CSensrvTestCases::FindChannels: Channel count: %d"), aChannelInfoList.Count() );
...
TInt CSensrvTestCases::OpenChannelL()
    ...
    if( err )
        {
        RDebug::Print( _L("CSensrvTestCases::OpenChannelL: FindChannels error: %d"), err );
        return err;
        }
...
TInt CSensrvTestCases::OpenClosePerformanceL()
    ...
    if( err )
        {
        RDebug::Print( _L("CSensrvTestCases::OpenClosePerformanceL: FindChannels error: %d"), err );
        return err;
        }
    if( iChannelInfoList.Count() == 0 )
        {
...
TInt CSensrvTestCases::StopDataListeningL()
    ...
    TBuf<16> location;
    location.Copy( iSensorChannel->GetChannelInfo().iLocation );
    RDebug::Print( _L("CSensrvTestCases::StopDataListeningL: Channel: %S"), &location );
...
TInt CSensrvTestCases::PanicClient()
    {
    RDebug::Print( _L("CSensrvTestCases::PanicClient") );
...
TInt CSensrvTestCases::WaitSSYUnloadingL()
    {
    RDebug::Print( _L("CSensrvTestCases::WaitSSYUnloadingL") );
    ...
    if ( err == KErrNone )
        ...
        if ( err == KErrNone )
            ...
            {
            RDebug::Print( _L( "CSensrvTestCases::WaitSSYUnloadingL - KSensrvSsyInactivityPeriodKey value getting failed: %d, using default value." ), err );
            }

        }
    else
        {
        RDebug::Print( _L( "CSensrvTestCases::WaitSSYUnloadingL - Settings repository opening failed: %d" ), err );
        }

    timeout += 2 * KSecondsToMicros;
    RDebug::Print( _L("CSensrvTestCases::WaitSSYUnloadingL - Wait %d microsecond...."), timeout );
    User::After( timeout );
    RDebug::Print( _L("CSensrvTestCases::WaitSSYUnloadingL - Wait completed") );

    TInt unloadingCount( 0 );
    err = RProperty::Get( KPSUidSensrvTest, KSensrvSSYUnloadingCount, unloadingCount );
    if( err == KErrNone )
        {
        RDebug::Print( _L("CSensrvTestCases::WaitSSYUnloadingL - SSY unloading count %d, expected %d"), unloadingCount, iExpectedUnloads );
        if( unloadingCount != iExpectedUnloads )
            {
            RDebug::Print( _L("CSensrvTestCases::WaitSSYUnloadingL - SSY unloading error") );
            err = KErrGeneral;
            }
        }
    else
        {
        RDebug::Print( _L("CSensrvTestCases::WaitSSYUnloadingL - Get KSensrvSSYUnloadingCount error %d"), err );
            }
...
TInt CSensrvTestCases::ParseChannelInfo( TSensrvChannelInfo& aChannelInfo, CStifItemParser& aItem )
    {
    RDebug::Print( _L("CSensrvTestCases::ParseChannelInfo") );
    ...
    for( TInt i = 1; i <= 5; ++i )
        ...
        if( err )
            {
            RDebug::Print( _L("CSensrvTestCases::ParseChannelInfo: parameter[%d] error: %d"), i, err );
            err = KErrNone;
            return err;
            }
        RDebug::Print( _L("CSensrvTestCases::ParseChannelInfo: parameter[%d] = %d"), i, param );
        array.Append( param );
        }
    ...
    RDebug::Print( _L("CSensrvTestCases::ParseChannelInfo: location = %S"), &location );
    ...
    RDebug::Print( _L("CSensrvTestCases::ParseChannelInfo: vendorId = %S"), &vendorId );
...
void CSensrvTestCases::DataReceived( CSensrvChannel& aChannel,
    ...
    if( &aChannel != iSensorChannel )
        {
        RDebug::Print( _L("CSensrvTestCases::DataReceived - Error: incorrect channel" ) );
        RProperty::Set( KPSUidSensrvTest, iCompletedKey, KErrArgument );
        }

    TBuf<16> location;
    location.Copy( aChannel.GetChannelInfo().iLocation );
    RDebug::Print( _L("CSensrvTestCases::DataReceived: Channel: %S"), &location );
    RDebug::Print( _L("CSensrvTestCases::DataReceived[%d]: aCount = %d, aDataLost = %d"), iDataListeningRepeatCounter, aCount, aDataLost );
    ...
    if( aChannel.GetChannelInfo().iChannelType == KSensrvChannelTypeIdAccelerometerXYZAxisData )
        ...
        if( overflowTest )
            ...
            if( err )
                {
                RDebug::Print( _L("CSensrvTestCases::DataReceived - GetData Error %d" ), err );
                iSensorChannel->StopDataListening();
                RProperty::Set( KPSUidSensrvTest, iCompletedKey, err );
                iTestCaseCompleted = ETrue;
                iMutex.Signal();
        ...
        for( TInt i = 0; i < aCount; i++ )
            ...
            if( iPreviousAxis_xData != KErrNotFound )
                ...
                if( i == 0 )
                    ...
                    if( iPreviousAxis_xData + aDataLost + 1 != data.iAxisX ||
                        ...
                        {
                        RDebug::Print( _L("CSensrvTestCases::DataReceived - Error: Data is invalid" ) );
            ...
            if( iGetDataDelay > 0 )
                {
                RDebug::Print( _L("CSensrvTestCases::DataReceived - read delay %d" ), iGetDataDelay * 1000 );
...
TInt CSensrvTestCases::GetPropertyL()
    {
   /*****************************
    * SDK example (GetProperty) *
    *****************************/
    /*
    TSensrvProperty property;
    TReal channelAccuracy( 0 );
    ...
    if( property.PropertyType() == ESensrvRealProperty )
        {
        property.GetValue( channelAccuracy );
        }
    /*
    /******************************* OR *******************************/
    /*
    RSensrvPropertyList channelAccuracyList;
    CleanupClosePushL( channelAccuracyList );

    iSensorChannel->GetAllPropertiesL( KSensrvPropIdChannelAccuracy, channelAccuracyList );

    for( TInt i=0 ; i<channelAccuracyList.Count() ; i++ )
        {
        if( KSensrvItemIndexNone == channelAccuracyList[i].PropertyItemIndex() )
            {
            if( property.PropertyType() == ESensrvRealProperty )
                {
                property.GetValue( channelAccuracy );
            ...
            {
            //Accuracy defined in channelAccuracyList[i] is channel item specific
            }
        }

    CleanupStack::PopAndDestroy( &channelAccuracyList ); //Close() is being called on "channelAccuracyList"
    */

   /*********************************
    * SDK example (GetProperty) end *
    *********************************/


    /****************************
    * SDK example (SetProperty) *
    *****************************/
    /*
    TSensrvProperty property;
    TInt err( KErrNone );
    ...
    */
    /********************************
    * SDK example (SetProperty) end *
    *********************************/


    /*******************************
    * SDK example (Array Property) *
    ********************************/
    /*
    TSensrvProperty property;
    TInt err( KErrNone );
    ...
    if( ESensrvArrayPropertyInfo == property.GetArrayIndex() )
        ...
        //which array index is declared in aray propertys value.
        TInt arrayIndex( 0 );
    ...
    */
    /***********************************
    * SDK example (Array Property) end *
    ************************************/


   /**************************************
    * SDK example (Scaling channel data) *
    **************************************/
/*
    TSensrvProperty property;
...
*/
    /******************************************
    * SDK example (Scaling channel data) end *
    ******************************************/
...
void CSensrvTestCases::PropertyChanged( CSensrvChannel& aChannel, const TSensrvProperty& aChangedProperty )
    ...
    TBuf<16> location;
    location.Copy( aChannel.GetChannelInfo().iLocation );
    RDebug::Print( _L("CSensrvTestCases::PropertyChanged: Channel: %S"), &location );
    switch( aChangedProperty.PropertyType() )
        ...
        case ESensrvIntProperty:
            ...
            RDebug::Print( _L("CSensrvTestCases::PropertyChanged: PropertyType = ESensrvIntProperty, value = %d"), value );
            break;
            }
        case ESensrvRealProperty:
            ...
            TReal value;
            aChangedProperty.GetValue( value );
            RDebug::Print( _L("CSensrvTestCases::PropertyChanged: PropertyType = ESensrvRealProperty, value = %f"), value );
            break;
            }
        case ESensrvBufferProperty:
            ...
            TBuf8<KSensrvPropertyTextBufferSize> value;
            aChangedProperty.GetValue( value );
            RDebug::Print( _L("CSensrvTestCases::PropertyChanged: PropertyType = ESensrvRealProperty, value = %S"), &value );
            break;
            }
        default:
            {
            RDebug::Print( _L("CSensrvTestCases::PropertyChanged: Unknown property type") );
            break;
            }
        }
...
void CSensrvTestCases::PropertyError( CSensrvChannel& aChannel, TSensrvErrorSeverity aError )
    ...
    TBuf<16> location;
    location.Copy( iSensorChannel->GetChannelInfo().iLocation );
    RDebug::Print( _L("CSensrvTestCases::PropertyError: Channel: %S, aError = %d"), &location, aError );
    if( &aChannel != iSensorChannel )
        {
        RDebug::Print( _L("CSensrvTestCases::PropertyError - Error: incorrect channel" ) );
        RProperty::Set( KPSUidSensrvTest, iCompletedKey, KErrArgument );
        }
...
void CSensrvTestCases::SetPropertySuccessIndicationChanged( TSetPropertySuccessIndicator aIndication )
    {
    iMutex.Wait();
    RDebug::Print( _L("CSensrvTestCases::SetPropertySuccessIndicationChanged: aIndication = %d"), aIndication );
...
TInt CSensrvTestCases::CreateNewProcessL()
    ...
    for( TInt i=0 ; i<iProcessCount ; i++ )
        ...
        parameter.Num( i+1 );
        RDebug::Print( _L("CSensrvTestCases::CreateNewProcessL: processName=%S parameter=%S"), &processName, &parameter );
...
TInt CSensrvTestCases::StopConditionListeningL()
    ...
    TBuf<16> location;
    location.Copy( iSensorChannel->GetChannelInfo().iLocation );
    RDebug::Print( _L("CSensrvTestCases::StopConditionListeningL: Channel: %S"), &location );
...
void CSensrvTestCases::ConditionMet( CSensrvChannel& aChannel,
    ...
    if( &aChannel != iSensorChannel )
        {
        RDebug::Print( _L("CSensrvTestCases::ConditionMet - Error: incorrect channel" ) );
        RProperty::Set( KPSUidSensrvTest, iCompletedKey, KErrArgument );
        }

    TBuf<16> location;
    location.Copy( aChannel.GetChannelInfo().iLocation );
    RDebug::Print( _L("CSensrvTestCases::ConditionMet: Channel: %S"), &location );
    RDebug::Print( _L("CSensrvTestCases::ConditionMet[%d]"), iConditionListeningRepeatCounter );
    ...
    if( aChannel.GetChannelInfo().iChannelType == KSensrvChannelTypeIdAccelerometerXYZAxisData )
        ...
        if ( sizeof(TSensrvAccelerometerAxisData) != aValue.Size() )
            ...
            RDebug::Print( _L("CSensrvTestCases::ConditionMet - sizeof(TSensrvAccelerometerAxisData): %d != aValue.Size(): %d"), sizeof(TSensrvAccelerometerAxisData), aValue.Size() );
            }
        }
    else if( aChannel.GetChannelInfo().iChannelType == KSensrvTestEventChannelType )
        {
        if ( sizeof(TSensrvTestEventData) != aValue.Size() )
            {
            RDebug::Print( _L("CSensrvTestCases::ConditionMet - sizeof(TSensrvTestEventData): %d != aValue.Size(): %d"), sizeof(TSensrvTestEventData), aValue.Size() );
            failedSizeCheck = ETrue;
            }
        }
    else if( aChannel.GetChannelInfo().iChannelType == KSensrvChannelTypeIdAccelerometerDoubleTappingData )
        ...
        if ( sizeof(TSensrvTappingData) != aValue.Size() )
            {
            RDebug::Print( _L("CSensrvTestCases::ConditionMet - sizeof(TSensrvTappingData): %d != aValue.Size(): %d"), sizeof(TSensrvTappingData), aValue.Size() );
            failedSizeCheck = ETrue;
            }
        }
    ...
    for ( TInt j = 0;
        ...
        // Check ConditionValidity-function
        TBool isValid(
            iConditionEvaluator->CheckConditionValidityL(
                condition1,
                condition2) );
        
        if (isValid)
            {
            RDebug::Print(_L("  ### Condition is valid"));
            }
        else
            {
            // Ok, not valid, do nothing
            }            
        }
    ...
    if (!((  singleMatch && matchCount)
          || (!singleMatch && matchCount == checkCount))   )
        {
        RDebug::Print( _L("CSensrvTestCases::ConditionMet - Error: Set does not match the data." ) );
        }
...
void CSensrvTestCases::ConditionError( CSensrvChannel& aChannel,
    ...
    TBuf<16> location;
    location.Copy( iSensorChannel->GetChannelInfo().iLocation );
    RDebug::Print( _L("CSensrvTestCases::ConditionError: Channel: %S, aError = %d"), &location, aError );
    if( &aChannel != iSensorChannel )
        {
        RDebug::Print( _L("CSensrvTestCases::ConditionError - Error: incorrect channel" ) );
        RProperty::Set( KPSUidSensrvTest, iCompletedKey, KErrArgument );
        }
...
TInt CSensrvTestCases::AdjustChannels(CStifItemParser& aItem)
    ...
    if( !err )
        ...
        RDebug::Print( _L("CSensrvTestCases::AdjustChannels: vendorId = %S"), &vendorId );
...
void CSensrvTestCases::ChannelChangeDetected( const TSensrvChannelInfo& aDetectedChannel,
    ...
    RDebug::Print(  _L( "CSensrvTestCases::ChannelChangeDetected: aChangeType: %d iChannelId: %d, iContextType: 0x%x, iQuantity: 0x%x, iChannelType: 0x%x, iLocation: \"%S\", iVendorId: \"%S\", iDataItemSize: %d " ),
                       aChangeType,
    ...
    if (iExpectedVendor.Compare(aDetectedChannel.iVendorId))
        {
        // Unexpected vendor
        RDebug::Print( _L("CSensrvTestCases::ChannelChangeDetected: ERROR: Unexpected vendor") );
        RProperty::Set( KPSUidSensrvTest, iCompletedKey, KErrArgument );
        iTestCaseCompleted = ETrue;
        }
    else
        ...
        if (iExpectedChannelChangeType == aChangeType)
            ...
            if( iChannelChangeListeningRepeatCounter == iChannelChangeListeningRepeatCount )
                ...
                RDebug::Print( _L("CSensrvTestCases::ChannelChangeDetected: ERROR: Unexpected notification") );
            ...
            RDebug::Print( _L("CSensrvTestCases::ChannelChangeDetected: ERROR: Unexpected change type: %d"), aChangeType );
            RProperty::Set( KPSUidSensrvTest, iCompletedKey, KErrArgument );
            iTestCaseCompleted = ETrue;
            }
        }
...
void CSensrvTestCases::ChannelDetectionError( CSensrvChannelFinder& aChannelFinder,
    ...
    {
    iMutex.Wait();
    RDebug::Print( _L("CSensrvTestCases::ChannelDetectionError: aError = %d"), aError );
    if( &aChannelFinder != iChannelFinder )
        {
        RDebug::Print( _L("CSensrvTestCases::ChannelDetectionError - Error: incorrect channelfinder reference" ) );
        RProperty::Set( KPSUidSensrvTest, iCompletedKey, KErrArgument );
        }
    ...
    if( aError == ESensrvErrorSeverityFatal )
        ...
        if ( err != KErrBadHandle )
            {
            RDebug::Print( _L("CSensrvTestCases::ChannelDetectionError - Error: Incorrect error at FindChannelsL check: %d" ), err );
            RProperty::Set( KPSUidSensrvTest, iCompletedKey, err );
            iTestCaseCompleted = ETrue;
            iMutex.Signal();
            return;
