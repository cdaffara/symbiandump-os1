...
TInt MainL()
	{
	RDebug::Print(_L("appfwk-sysmon-0037: MainL"));
    ...
    switch (runCount)
		...
		case 0:
			...
			CStartupProperties* props = CStartupProperties::NewLC(_L("tsysmon_app_failcount2failquick_slave.exe"), _L("APPFWK-SYSMON-0037"));
			props->SetMonitored(ETrue);
			props->SetStartupType(EStartProcess);
			props->SetStartMethod(EWaitForStart);
			props->SetNoOfRetries(2);
			props->SetTimeout(0);
			props->SetRecoveryParams(EIgnoreOnFailure, 0);

			RProcess slave1;
			slave1.Create(_L("tsysmon_app_failcount2failquick_slave.exe"), _L("APPFWK-SYSMON-0037"));
			CleanupClosePushL(slave1);
			slave1.Resume();

			// Register with SysMon
			sysmon.MonitorL(*props, slave1);
			User::After(500000); //Allow slave to catch up
	...
	RDebug::Print(_L("appfwk-sysmon-0037: End MainL"));
	return KErrNone;
	}
...
GLDEF_C TInt E32Main()
	...
	RDebug::Print(_L("appfwk-sysmon-0037: E32Main"));
	__UHEAP_MARK;
	...
	delete cleanup;
	__UHEAP_MARKEND;
	RDebug::Print(_L("appfwk-sysmon-0037: End E32Main"));
	return KErrNone;
