...
New Test CaseID 		DEVSRVS-SHMA-STARTSAFE-0011
 */
...
TVerdict CAppFwkStartSafeTestStepProcStartMon::doTestStepL( void )
	...
	if( (KErrNone == err) && ProcessExists(namePtr) )
		...
		RSemaphore sem;
		err = sem.CreateGlobal(KStartShmaTestProcGood, 0);
		INFO_PRINTF2( _L("Semaphore created with %d err"),err);
		TEST(KErrNone == err);
		...
		if( ProcessExists( namePtr, proc ) )
			...
			TEST(KErrNone == testProc.Connect());
