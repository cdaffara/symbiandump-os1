...
@publishedPartner
@deprecated
@panic EInvalidCommandType if the Command type is invalid
@panic EInvalidStartMethod if the start method is invalid
@panic EInvalidTimeout if start method is not EWaitForStart then timeout value should be zero
@panic EInvalidNoOfRetries if start method is not EWaitForStart then no_of_retries should be zero
@panic EInvalidRecoveryMethod if the Recovery method called after a command failure is invalid
*/
