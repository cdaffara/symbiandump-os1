// Copyright (c) 1999-2009 Nokia Corporation and/or its subsidiary(-ies).
// All rights reserved.
// This component and the accompanying materials are made available
// under the terms of "Eclipse Public License v1.0"
// which accompanies this distribution, and is available
// at the URL "http://www.eclipse.org/legal/epl-v10.html".
//
// Initial Contributors:
// Nokia Corporation - initial contribution.
//
// Contributors:
//
// Description:
// Calculation for the RFCOMM (TS07.10) FCS
// 
//

#include <e32std.h>
#include "rfcommfcs.h"
#include "debug.h"

#pragma warning( disable : 4244) // Conv from int to char

/*
  This table is lifted from the TS07.10 spec in the appendices.
*/

static const TUint8 crctable[256] = { //reversed, 8-bit, poly=
};


TUint8 CalcFCS(const TUint8* aData, TInt aLen)
	/**
	   Calculates the FCS over data starting at aData and of length
	   aLen.  Uses the algorithm from the TS07.10 spec.
	**/
	{
	TUint8 fcs=0xff;
	
	while(aLen--)
		{
		fcs=crctable[fcs^*aData++];
		}
	
	fcs=~fcs;
	return fcs;
	}

TBool CheckFCS(const TUint8* aData, TInt aLen, TUint8 aFCS)
    /**
	   Check a received FCS.
	 **/   
	{
	TUint8 fcs=0xFF;
	while (aLen--)
		{
		fcs=crctable[fcs^*aData++];
		}
    /*Ones complement*/
	fcs=crctable[fcs^aFCS];
// log "PDU FCS:" aFCS - Shehla
	RFQBLOG(KBlogTypeFCS, Log(KBlogActualFCS, aFCS));
// log this as "Calculated FCS (should be 0xCF): " fcs - Shehla
	RFQBLOG(KBlogTypeFCS, Log(KBlogComputedFCS, fcs));
    /*0xCF is the reversed order of 11110011.*/
	TBool result=EFalse; //- Shehla
	if (fcs==0xCF)
		//return ETrue;
		result=ETrue;
	else 
		//return EFalse;
		result=EFalse;
//log "FCS Computation" result - Shehla
	RFQBLOG(KBlogTypeFCS, Log(KBlogComparedFCS, result));

	return result;
}
