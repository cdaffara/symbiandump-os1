// Copyright (c) 2009 Nokia Corporation and/or its subsidiary(-ies).
// All rights reserved.
// This component and the accompanying materials are made available
// under the terms of "Eclipse Public License v1.0"
// which accompanies this distribution, and is available
// at the URL "http://www.eclipse.org/legal/epl-v10.html".
//
// Initial Contributors:
// Nokia Corporation - initial contribution.
//
// Contributors:
//
// Description:
//

#include "stresscrp.h"
#define SYMBIAN_GRAPHICS_GCE

#include <graphics/wsgraphicscontext.h>
#include <graphics/wsgraphicdrawerinterface.h>

void StressCrpNgaContext::Draw(MWsGc& aGc, const TRect& aRect)
	{
	MWsGraphicsContext* context = static_cast<MWsGraphicsContext*>(aGc.ResolveObjectInterface(KMWsGraphicsContext));	
	context->SetDrawMode(MWsGraphicsContext::EDrawModePEN);
	context->SetPenColor(TRgb(255,0,0));
	context->SetPenStyle(MWsGraphicsContext::ESolidPen);
	context->SetBrushStyle(MWsGraphicsContext::ESolidBrush);
	context->SetBrushColor(TRgb(255,0,0));
	context->DrawRect(aRect);
	context->SetBrushColor(TRgb(0,0,255));
	context->DrawEllipse(aRect);
	}
