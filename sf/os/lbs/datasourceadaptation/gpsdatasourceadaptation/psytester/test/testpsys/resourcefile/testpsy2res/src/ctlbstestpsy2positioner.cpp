...
void CT_LbsTestPsy2Positioner::NotifyPositionUpdate(TPositionInfoBase& aPosInfo, TRequestStatus& aStatus)
	...
	if (++iRequests == 1)
        ...
        RThread thread;
	    TBuf<32> threadName;
	    threadName.Format(KThreadName, 1+iThreads.Count());
		TInt err;
        err = thread.Create(threadName, ThreadFunction, KDefaultStackSize, KMinHeapSize, KMinHeapSize, NULL);
        if(KErrNone == err)
        	{
        	err=iThreads.Append(thread);
        	}
		}
