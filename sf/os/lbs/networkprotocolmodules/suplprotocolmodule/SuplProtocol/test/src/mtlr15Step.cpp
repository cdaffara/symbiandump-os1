 ...
 */
TVerdict Cmtlr15Step::doTestStepPreambleL()
	{
...
/** Perform Cmtlr15Step test step.
This test verifies that the SUPL Protocol Module correctly handles 
a SUPL INIT message containing a request for Terminal Assisted A-GPS positioning
when LBS does not support Terminal Assisted A-GPS

@return TVerdict test result code
*/
TVerdict Cmtlr15Step::doTestStepL()
	{
	INFO_PRINTF1(_L("\t********************************************************************"));
	INFO_PRINTF1(_L("\tMTLR - agps Terminal Assisted requested / not supported"));
	INFO_PRINTF1(_L("\t********************************************************************"));
	INFO_PRINTF1(_L("- START -"));	
	
	
	// update the capabilities that LBS will report
	...
	posMethods[1].SetPosMethod(KLbsPositioningMeansGps, TPositionModuleInfo::ETechnologyTerminal);
	capabilities.SetPosMethods(posMethods, numMethods);
	iGatewayObserver->SetCurrentCapabilities(capabilities);
		
	
	// Initiate MT-LR
	// Generate received SUPL INIT message (test message content #3)
 ...
 * Override of base class virtual
 */
TVerdict Cmtlr15Step::doTestStepPostambleL()
	{
	delete iModule;
...
*/
void Cmtlr15Step::OnSuplInitComplete(TLbsSuplPushChannel /*aChannel*/, TLbsSuplPushRequestId /*aReqId*/, TInt aError, TInt /*aReserved*/)
	{
