 ...
 */
TVerdict Cmtlr7Step::doTestStepPreambleL()
	{
...
/** Perform Cmtlr7Step test step.

This test verifies that the SUPL Protocol Module correctly handles an MT-LR 
when the requested positioning type is Terminal Assisted 
(and Terminal Assisted Positioning is supported)

@return TVerdict test result code
*/
TVerdict Cmtlr7Step::doTestStepL()
	{
	INFO_PRINTF1(_L("\t********************************************************************"));
	...
	// Initiate MT-LR
	// Generate received SUPL INIT message (test message content #3 - terminal assisted)
	TPtr8 messagePtr(iReceiveBuffer.Des());
	TInt err = iSuplInitGenerator->GenerateSuplInitL(3, messagePtr);
	if (err != KErrNone)
		{
	...
	suplPos = BuildSuplPosRequestL(TPositionModuleInfo::ETechnologyTerminal | TPositionModuleInfo::ETechnologyAssisted, EAssistanceDataNone);
	iNetworkObserver->InjectSuplMessage(iNetworkObserver->SessionId(), suplPos);	
	...
	// Fake LBS sending results (measurements) to SPM
	INFO_PRINTF1(_L("\tLBS -> RespondLocationRequest"));
	TPositionExtendedSatelliteInfo satelliteInfo;
	TSatelliteData satelliteData;
	satelliteData.SetSatelliteId(23);
	satelliteData.SetAzimuth(32);
	satelliteData.SetElevation(64);
	satelliteData.SetIsUsed(ETrue);
	satelliteData.SetSignalStrength(15);	
	satelliteInfo.AppendSatelliteData(satelliteData);
	iModule->RespondLocationRequest(iGatewayObserver->SessionIdValue(),KErrNone,satelliteInfo);
	...
	// Check gateway receives a location update (due to the received final position)
	INFO_PRINTF1(_L("\tLBS <- ProcessLocationUpdate()"));
	if (EFail == CheckGatewayCallbackL(CSuplGatewayObserver::EProcessLocationUpdate))
		{
		SetTestStepResult(EFail);
		return TestStepResult();
		}
 ...
 * Override of base class virtual
 */
TVerdict Cmtlr7Step::doTestStepPostambleL()
	{
	delete iModule;
...
*/
void Cmtlr7Step::OnSuplInitComplete(TLbsSuplPushChannel /*aChannel*/, TLbsSuplPushRequestId /*aReqId*/, TInt aError, TInt /*aReserved*/)
	{
