 ...
 */
TVerdict Cmtlr11Step::doTestStepPreambleL()
	{
...
/** Perform Cmtlr11Step test step.

This test verifies that the SUPL Protocol Module correctly handles 
an MT-LR SUPL INIT with the optional notificaion element absent.

@return TVerdict test result code
*/
TVerdict Cmtlr11Step::doTestStepL()
	{
	INFO_PRINTF1(_L("\t********************************************************************"));
	INFO_PRINTF1(_L("\tMTLR - notification element of SUPL INIT missing"));
	INFO_PRINTF1(_L("\t********************************************************************"));
	INFO_PRINTF1(_L("- START -"));	
	

	// Initiate MT-LR
	// Generate received SUPL INIT message (test message content #7)
	TPtr8 messagePtr(iReceiveBuffer.Des());
	TInt err = iSuplInitGenerator->GenerateSuplInitL(7, messagePtr);
	if (err != KErrNone)
		{
 ...
 * Override of base class virtual
 */
TVerdict Cmtlr11Step::doTestStepPostambleL()
	{
	delete iModule;
...
*/
void Cmtlr11Step::OnSuplInitComplete(TLbsSuplPushChannel /*aChannel*/, TLbsSuplPushRequestId /*aReqId*/, TInt aError, TInt /*aReserved*/)
	{
