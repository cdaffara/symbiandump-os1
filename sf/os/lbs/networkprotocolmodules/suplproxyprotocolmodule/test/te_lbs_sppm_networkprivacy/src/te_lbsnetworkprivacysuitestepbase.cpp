 ...
 */
	...
	//Initialisation of NRH<->NG & SystemController interfaces  
    ...
    iPosHandler = COMASuplPosHandlerBase::NewL(KOMASuplHandlerUid);
    ...
    if (status.Int() != KErrNone)
        {
        INFO_PRINTF1(_L("InitializeL() failed!\n"));
		...
		if(iPosSession == NULL)
			{
			INFO_PRINTF1(_L("CreateNewSessionL() failed!\n"));
			SetTestStepResult(EFail);
			}
		else
			{
 ...
 */
TReal CTe_LbsNetworkPrivacySuiteStepBase::SuplVersion()
	{
 ...
 */
void CTe_LbsNetworkPrivacySuiteStepBase::TerminateSession()
	{
