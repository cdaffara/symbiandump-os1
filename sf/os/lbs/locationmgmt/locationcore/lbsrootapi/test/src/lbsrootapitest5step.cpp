...
CLbsRootApiTest5Step::~CLbsRootApiTest5Step()
/**
 * Destructor
 */
	{
...
CLbsRootApiTest5Step::CLbsRootApiTest5Step() : iCount(0)
/**
 * Constructor
 */
	{
	...
	SetTestStepName(KLbsRootApiTest5Step);
	}

TVerdict CLbsRootApiTest5Step::doTestStepPreambleL()
/**
 * @return - TVerdict code
 * Override of base class virtual
 */
	{
	INFO_PRINTF1(_L("I am in Test Step Preamble in Class CLbsRootApiTest5Step"));
...
__UHEAP_MARK;
	...
	INFO_PRINTF1(_L("CLbsRootApiTest5Step : open RLbsSystemController"));

 	SetTestStepResult(EPass);
	return TestStepResult();
	}

void CLbsRootApiTest5Step::CheckProcessesAreUp()
	{
	RProcess process;
...
TVerdict CLbsRootApiTest5Step::doTestStepL()
/**
 * @return - TVerdict code
 ...
 */
	...
	if (TestStepResult()==EPass)
		...
		for(TInt count = 0; count < 100 && TestStepResult() == EPass; ++count)
			{
			INFO_PRINTF2(_L("Start & Stop: iteration %d/100"), count+1);
			TInt err = iLbsRootProcessApi.RequestSystemStartup();		
			if(err != ELbsStartupSuccess)
				{
				INFO_PRINTF2(_L("CLbsRootApiTest5Step : Failed to start system: %d"), err);    	
				SetTestStepResult(EFail);
				}
			...
			if(TestStepResult() == EPass)
				...
				if(TestStepResult() == EPass)
					{
					INFO_PRINTF1(_L("CLbsRootApiTest5Step : Processes up OK"));    	
					}

				INFO_PRINTF1(_L("CLbsRootApiTest5Step : Panic and immediately request system closeDown from the test harness"));
    			iPanicRequest.Set(1);        
...
TVerdict CLbsRootApiTest5Step::doTestStepPostambleL()
/**
 * @return - TVerdict code
 * Override of base class virtual
 */
	{
	INFO_PRINTF1(_L("I am in Test Step Postamble in Class CLbsRootApiTest5Step"));
