...
/**
 @file TestSelfLocateLogStep.cpp
*/
...
CTestSelfLocateLogStep::~CTestSelfLocateLogStep()
/**
 * Destructor
 */
	{
 ...
 */
	...
	SetTestStepName(KTestSelfLocateLogStep);
	}

TVerdict CTestSelfLocateLogStep::doTestStepPreambleL()
/**
...
TVerdict CTestSelfLocateLogStep::doTestStepL()
/**
 * @return - TVerdict code
 ...
 */
	{
	  if (TestStepResult()==EPass)
		{
		__UHEAP_MARK;
		INFO_PRINTF1(_L("CTestSelfLocateLogStep::doTestStepL() in the file TestSelfLocateLogStep.cpp"));  //Block start
		TInt TheLocalAppUid;
		TInt TheCost;
		...
		if(read)
			...
			CLbsSelfLocateLogEvent* event = CLbsSelfLocateLogEvent::NewL(TUid::Uid(0), NULL);
			event->SetLocalApp(TUid::Uid(TheLocalAppUid));
			...
			if(ErrorCode == KErrNone)
				{
				INFO_PRINTF1(_L("\t SelfLocate Event Logged in."));
				SetTestStepResult(EPass);
				}
			else
				{
				INFO_PRINTF1(_L("\t Error: SelfLocate Logging Event."));
			...
			// add the event using the same event object
			...
			filter->SetEventType(TUid::Uid(KLogLbsSelfLocateEventType));
			...
			TPosition pos;
			TPositionInfoBase* base = lbsLogEvent->PositionInfo();
			if(base != NULL)
				{
				static_cast<TPositionInfo*>(base)->GetPosition(pos);	
				}
			else
				{
				INFO_PRINTF1(_L("\t Error: Read Position Info failed."));
				SetTestStepResult(EFail);
				return TestStepResult();
				}
							
			read = ETrue;
			CLbsSelfLocateLogEvent* selfEvent = static_cast<CLbsSelfLocateLogEvent*>(lbsLogEvent);
			read &= (selfEvent->LocalApp() == TUid::Uid(TheLocalAppUid));
			read &= (lbsLogEvent->CostInformation() == (TBool)TheCost);
			read &= (lbsLogEvent->EventType() == TUid::Uid(KLogLbsSelfLocateEventType));
			...
			SetTestStepResult(EPass);
			//-------------------------------------------------------------
			CleanupStack::PopAndDestroy(lbsLogEvent);// lbslogEvent
			CleanupStack::PopAndDestroy(logEvent);// logEvent
			CleanupStack::PopAndDestroy(filter);// active
			CleanupStack::PopAndDestroy(view);// filter
			CleanupStack::PopAndDestroy(active);// view
...
TVerdict CTestSelfLocateLogStep::doTestStepPostambleL()
/**
 * @return - TVerdict code
 * Override of base class virtual
 */
