...
//
// Description:
// @file ctlbsconflictstepx3pushtimer.cpp
...
Static Constructor
*/
CT_LbsConflictStep_X3PPushTimer* CT_LbsConflictStep_X3PPushTimer::New(CT_LbsConflictServer& aParent)
	{
	// Note the lack of ELeave.
	// This means that having insufficient memory will return NULL;
	CT_LbsConflictStep_X3PPushTimer* testStep = new CT_LbsConflictStep_X3PPushTimer(aParent);
	if (testStep)
		{
		TInt err = KErrNone;
 ...
 * Constructor
 */
CT_LbsConflictStep_X3PPushTimer::CT_LbsConflictStep_X3PPushTimer(CT_LbsConflictServer& aParent) : CT_LbsConflictStep(aParent)
	{
	SetTestStepName(KLbsConflictStep_X3PPushTimer);
	}


void CT_LbsConflictStep_X3PPushTimer::ConstructL()
	{
 ...
 * Destructor
 */
CT_LbsConflictStep_X3PPushTimer::~CT_LbsConflictStep_X3PPushTimer()
	{
	iDoX3PTimer->Cancel();
	delete iDoX3PTimer;
...
*/
void CT_LbsConflictStep_X3PPushTimer::SetExpectedSeq()
	{
	...
	SetVerifySeqEvent(EX3P_TIMER_MOLR_NetSim_Got_NotifyFacilityLcsMoLrResult);
...
*/
void CT_LbsConflictStep_X3PPushTimer::VerifyPosInfos()
	{
	T_LbsUtils utils;
	...
	// Verify entry 1 for the X3P TIMER. 
	// We expect a real location value, compare using the data
	// sent to the test APGS module.
...
*/
void CT_LbsConflictStep_X3PPushTimer::StartX3PL(TInt aTransmitId)
	{
	INFO_PRINTF1(_L("&gt;&gt;CT_LbsConflictStep_X3PPushTimer::StartX3PL()"));
	...
	if (aTransmitId == KLbsConflictX3PTimerId)
		{	
		INFO_PRINTF1(_L("Starting X3P(TIMER)"));
...
/** Run the test.
 */
TVerdict CT_LbsConflictStep_X3PPushTimer::doTestStepL()
	{
	INFO_PRINTF1(_L("&gt;&gt;CT_LbsConflictStep_X3PPushTimer::doTestStepL()"));
...
*/
void CT_LbsConflictStep_X3PPushTimer::Connected()
	{
...
void CT_LbsConflictStep_X3PPushTimer::Disconnected()
	{
	// Call base implementation.
...
void CT_LbsConflictStep_X3PPushTimer::NotifyRegisterLcsMoLr(const TDesC& aData)
	{
	...
	if (aData != KNullDesC)
		...
		// Process X3P Timer register.			
			...
			{
			INFO_PRINTF1(_L("Got - X3P(TIMER) MOLR - NetSim Notify Register Lcs MoLr - Event."));
...
void CT_LbsConflictStep_X3PPushTimer::NotifyReleaseLcsMoLr(TInt aReason)
	{
...
void CT_LbsConflictStep_X3PPushTimer::NotifyMeasurementControlLocation(const TPositionInfo& aPosition, 
	...
	if (iState == EReqX3PTimer)
		{
		INFO_PRINTF1(_L("Got - X3P(TIMER) MOLR - NetSim Notify Measurement Control Location - Event."));
...
void CT_LbsConflictStep_X3PPushTimer::NotifyReleaseLcsLocationNotification(const CLbsNetworkProtocolBase::TLbsPrivacyResponse& aResult)
	{
	(void)aResult;
...
void CT_LbsConflictStep_X3PPushTimer::NotifyFacilityLcsMoLrResult(TInt aReason, const TPositionInfo& aPosition)
	{
	// Determine and record sequence event.
	if (iState == EReqX3PTimer)
		{
...
void CT_LbsConflictStep_X3PPushTimer::NotifyMeasurementReportLocation(const TPositionInfo& aPosition)
	{	
	// Determine and record sequence event.
	if (iState == EReqX3PTimer)
		{
...
void CT_LbsConflictStep_X3PPushTimer::NotifyMeasurementReportRequestMoreAssistanceData(const TLbsAssistanceDataGroup& aFilter)
	{
	(void)aFilter;
...
void CT_LbsConflictStep_X3PPushTimer::NotifyMeasurementReportControlFailure(TInt aReason)
	{
	// Unexpected callback for this test, record the sequence event to fail test.
	if (iState == EReqX3PTimer)
		{
...
*/	
void CT_LbsConflictStep_X3PPushTimer::MT_LbsDoX3PCallback(TInt aTransmitId, TRequestStatus& aStatus)
	{
	TInt expectedErr = KErrNone;
