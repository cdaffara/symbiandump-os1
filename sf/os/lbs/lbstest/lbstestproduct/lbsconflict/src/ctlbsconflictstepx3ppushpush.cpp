...
//
// Description:
// @file ctlbsconflictstepx3pushpush.cpp
...
Static Constructor
*/
CT_LbsConflictStep_X3PPushPush* CT_LbsConflictStep_X3PPushPush::New(CT_LbsConflictServer& aParent)
	{
	// Note the lack of ELeave.
	// This means that having insufficient memory will return NULL;
	CT_LbsConflictStep_X3PPushPush* testStep = new CT_LbsConflictStep_X3PPushPush(aParent);
	if (testStep)
		{
		TInt err = KErrNone;
 ...
 * Constructor
 */
CT_LbsConflictStep_X3PPushPush::CT_LbsConflictStep_X3PPushPush(CT_LbsConflictServer& aParent) : CT_LbsConflictStep(aParent)
	{
	SetTestStepName(KLbsConflictStep_X3PPushPush);
	}


void CT_LbsConflictStep_X3PPushPush::ConstructL()
	{
	...
	iDoX3PPush2 = CT_LbsDoX3P::NewL(this, KLbsConflictX3PPush2Id);
	}
 ...
 * Destructor
 */
CT_LbsConflictStep_X3PPushPush::~CT_LbsConflictStep_X3PPushPush()
	{
	iDoX3PPush->Cancel();
	delete iDoX3PPush;
...
*/
void CT_LbsConflictStep_X3PPushPush::SetExpectedSeq()
	{
	...
	// followed by a X3P Push2 (after X3P Push register iState = ERegX3PPush),
	// X3P(Push2) should be queued and handled after the X3P(Push)
	SetVerifySeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyRegisterLcsMoLr);				// --> To Net	
	SetVerifySeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyMeasurementControlLocation);	// <-- From Net
	SetVerifySeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyMeasurementReportLocation);	// --> To Net
	SetVerifySeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyFacilityLcsMoLrResult);

	SetVerifySeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyReleaseLcsMoLr);				// --> To Net
...
*/
void CT_LbsConflictStep_X3PPushPush::VerifyPosInfos()
	{
	T_LbsUtils utils;
	...
	// Verify entry 1 for the X3P(PUSH2). 
	// We expect a real location value, compare using the data
	// sent to the test APGS module.
...
*/
void CT_LbsConflictStep_X3PPushPush::StartX3PL(TInt aTransmitId)
	{
	INFO_PRINTF1(_L("&gt;&gt;CT_LbsConflictStep_X3PPushPush::StartX3PL()"));
	...
	// Push.	
	if (aTransmitId == KLbsConflictX3PPushId)
		{	
		INFO_PRINTF1(_L("Starting X3P(PUSH2)"));
		iDoX3PPush->SetOptions(transOpts);
		iDoX3PPush->StartL(KTransmitPushDestination, KTransmitPushPriority, *posInfo);
		}
		
	// Push.
	else if (aTransmitId == KLbsConflictX3PPush2Id)
		{
		INFO_PRINTF1(_L("Starting X3P(PUSH)"));
		iDoX3PPush2->SetOptions(transOpts);
		iDoX3PPush2->StartL(KTransmitPush2Destination, KTransmitPush2Priority, *posInfo);
		}
...
/** Run the test.
 */
TVerdict CT_LbsConflictStep_X3PPushPush::doTestStepL()
	{
	INFO_PRINTF1(_L("&gt;&gt;CT_LbsConflictStep_X3PPushPush::doTestStepL()"));
...
*/
void CT_LbsConflictStep_X3PPushPush::Connected()
	{
...
void CT_LbsConflictStep_X3PPushPush::Disconnected()
	{
	// Call base implementation.
...
void CT_LbsConflictStep_X3PPushPush::NotifyRegisterLcsMoLr(const TDesC& aData)
	{
	...
	if (aData != KNullDesC)
		...
		if (iState == EReqX3PPush)
			...
			// Start X3P Push2 (second X3P).
			StartX3PL(KLbsConflictX3PPush2Id);
			}

		// Process X3P Push register.			
		else
			{
			INFO_PRINTF1(_L("Got - X3P(PUSH2) MOLR - NetSim Notify Register Lcs MoLr - Event."));
			SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyRegisterLcsMoLr);

			// Verify telephone number.
			if (aData.Compare(KTransmitPush2Destination))
				{
				INFO_PRINTF1(_L("Failed test, bad X3P Push register data."));
				SetTestStepResult(EFail);
				}
...
void CT_LbsConflictStep_X3PPushPush::NotifyReleaseLcsMoLr(TInt aReason)
	{
	TInt expectedErr = KErrNone;

	// Determine and set sequence event.
	if (iState == EReqX3PPush2)
		{
		INFO_PRINTF1(_L("Got - X3P(PUSH2) MOLR - NetSim Notify Release Lcs MoLr - Event."));
		SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyReleaseLcsMoLr);
...
void CT_LbsConflictStep_X3PPushPush::NotifyMeasurementControlLocation(const TPositionInfo& aPosition, 
	...
	else if(iState == EReqX3PPush2)
		{
		INFO_PRINTF1(_L("Got - X3P(PUSH2) MOLR - NetSim Notify Measurement Control Location - Event."));
		SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyMeasurementControlLocation);
		}
...
void CT_LbsConflictStep_X3PPushPush::NotifyReleaseLcsLocationNotification(const CLbsNetworkProtocolBase::TLbsPrivacyResponse& aResult)
	{
	(void)aResult;
	...
	if (iState == EReqX3PPush)
		...
		{
		INFO_PRINTF1(_L("Got - X3P(PUSH2) MOLR - NetSim Notify Release Lcs Location Notification - Event."));
		//SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyReleaseLcsLocationNotification);
		}
	}


void CT_LbsConflictStep_X3PPushPush::NotifyFacilityLcsMoLrResult(TInt aReason, const TPositionInfo& aPosition)
	{
	// Determine and record sequence event.
	if (iState == EReqX3PPush)
		{
		...
		{
		INFO_PRINTF1(_L("Got - X3P(PUSH2) MOLR - NetSim Notify Facility Lcs MoLr Result - Event."));
		SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyFacilityLcsMoLrResult);
		}
...
void CT_LbsConflictStep_X3PPushPush::NotifyMeasurementReportLocation(const TPositionInfo& aPosition)
	{	
	// Determine and record sequence event.
	if (iState == EReqX3PPush)
		{
		...
		{
		INFO_PRINTF1(_L("Got - X3P(PUSH2) MOLR - NetSim Notify Measurement Report - Event."));
		SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyMeasurementReportLocation);
		}
...
void CT_LbsConflictStep_X3PPushPush::NotifyMeasurementReportRequestMoreAssistanceData(const TLbsAssistanceDataGroup& aFilter)
	{
	(void)aFilter;
	...
	if (iState == EReqX3PPush)
		...
		{
		INFO_PRINTF1(_L("Got - X3P(PUSH2) MOLR - NetSim Notify Measurement Report Request More Assistance Data - Event."));
		SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyMeasurementReportRequestMoreAssistanceData);
		}
	}


void CT_LbsConflictStep_X3PPushPush::NotifyMeasurementReportControlFailure(TInt aReason)
	{
	// Unexpected callback for this test, record the sequence event to fail test.
	if (iState == EReqX3PPush)
		{
		...
		{
		INFO_PRINTF2(_L("Got - X3P(PUSH2) MOLR - Net Sim Notify Measurement Report Control Failure - Event. Reason = %d"), aReason);
		SetCurrentSeqEvent(EX3P_PUSH2_MOLR_NetSim_Got_NotifyMeasurementReportControlFailure);
		}
	}
...
*/	
void CT_LbsConflictStep_X3PPushPush::MT_LbsDoX3PCallback(TInt aTransmitId, TRequestStatus& aStatus)
	{
	TInt expectedErr = KErrNone;
...
//		expectedErr = KErrPositionHighPriorityReceive;
		...
		{
		INFO_PRINTF1(_L("Got - Client X3P(PUSH2) Complete - Callback Event."));
		}
