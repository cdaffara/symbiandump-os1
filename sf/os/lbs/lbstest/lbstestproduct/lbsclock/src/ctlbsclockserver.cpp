...
// Header files of LBS Clock API Test Steps
...
LOCAL_C void MainL()
	...
	CT_LbsClockServer* server = CT_LbsClockServer::NewL();
...
/**
  ...
  @post   None
*/
CT_LbsClockServer* CT_LbsClockServer::NewL()
	{
	CT_LbsClockServer*server = new(ELeave) CT_LbsClockServer();
	CleanupStack::PushL(server);
	server->ConstructL(KLbsClockTestServer);
	CleanupStack::Pop(server);
	return server;
	}


CT_LbsClockServer::~CT_LbsClockServer()
	{
	}
...
/**
  ...
  @postcondition : none
*/
void CT_LbsClockServer::ConstructL(const TDesC& aName)
	{
	CT_LbsServer::ConstructL(aName);
	
	iSharedData = CT_LbsClockSharedData::NewL();
	}
...
/**
  ...
  @postcondition : none
*/
CTestStep* CT_LbsClockServer::CreateTestStep(const TDesC& aStepName)
	{
	// NULL if insufficient memory. This suits the API.

	// Test case test steps, each test step supports one or more test cases.
	if(aStepName == KLbsClockStep_SelectPlugin)	
		{
		return CT_LbsClockStep_SelectPlugin::New(*this);
		}

	else if(aStepName == KLbsClockStep_Main)	
		{
		return CT_LbsClockStep_Main::New(*this);
		}
